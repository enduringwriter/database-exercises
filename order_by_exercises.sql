...
SELECT COUNT(*) FROM employees WHERE first_name IN ('Irena', 'Vidya', 'Maya') ORDER BY first_name;
...
-- first name in table is Vidya Awdeh, last name is Maya Melter
SELECT first_name, last_name FROM employees WHERE first_name = 'Irena' OR first_name = 'Vidya' OR first_name = 'Maya' GROUP BY first_name, last_name;


-- ? how to use max min instead of scrolling?
-- or just use ASC/DESC limit 1
...
SELECT * FROM employees WHERE hire_date LIKE '199%' AND birth_date LIKE '%%%%-12-25' ORDER BY birth_date ASC, hire_date ASC;
